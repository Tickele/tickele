...
var ScreenWidget = PosBaseWidget.extend({
    ...
    barcode_product_action: function(code){
        ...
        if (self.pos.scan_product(code)) {
			...
			if(code['code'].charAt(0)=='9'){
				tickele_code=code['code'];
				self.gui.show_popup('confirm',{
                        'title': 'Usuario tickele aceptado',
                        'body': 'Se ha añadido el usuario tickele al carro, no se imprimira el ticket',
                    });
...
var ClientListScreenWidget = ScreenWidget.extend({
    ...
    show: function(){
        ...
        this.$('.new-customer').click(function(){
			...
			$('.client-name').val('Angel');
			$('.client-address-street').val('Angel');
			$('.client-address-street').text('Angel');
...
var PaymentScreenWidget = ScreenWidget.extend({
    ...
    //Posible sitio para llamar al archivo php.
	//console.log(order.get_orderlines());
	/*console.log(typeof tickele_code != "undefined");
	console.log(tickele_code != null);
	console.log(tickele_code.localeCompare("null") != 0);*/
	
	//if (typeof tickele_code != "undefined" && tickele_code != null && tickele_code.localeCompare("null") != 0){
		
		console.log('Consola');
		//console.log(order);
		//console.log('JSON');
		console.log(order.export_as_JSON());
		console.log('ID tickele: ' + tickele_code);
		console.log('Orderline');
		console.log(order.get_orderlines());
		console.log('Order selecionado');
		console.log(order.get_orderlines()[0]['product']['display_name']);
		console.log(order.get_orderlines()[0]['product']['price']);
		console.log(order.get_orderlines()[0]['product']['id']);
		console.log(order.get_orderlines()[0]['quantity']);
		var result = order.export_as_JSON();
		//console.log(JSON.stringify({amount_total:result['amount_total'], amount_return:result['amount_return'], name:result['uid'], fcm_id:tickele_code}));
		jQuery.ajax({
		type: "POST",
		url: 'http://api.tickele.com/v1/push/send',
		dataType: 'json',
		data: {amount_total:result['amount_total'], amount_return:result['amount_return'], name:result['uid'], fcm_id:tickele_code} ,
		beforeSend: function () {
		...
		jQuery.ajax({
		type: "POST",
		url: 'http://api.tickele.com/v1/ticket/create',
		dataType: 'json',
		data: { amount_total:result['amount_total'], amount_return:result['amount_return'], amount_tax:result['amount_tax'], cashier_name:"Angel", ticket_barcode:result['uid'], payment_type:1, amount_paid:result['amount_paid'], user_barcode:tickele_code} ,
		beforeSend: function () {
				console.log("Procesando, espere por favor...");
		},
		success:  function (response) {
		...
		var jason = order.export_as_JSON();
		for (i=0; i<order.get_orderlines().length;i++){
			//console.log(JSON.stringify({ticket_barcode: 1231232, price:order.get_orderlines()[i]['product']['price'], product_barcode:order.get_orderlines()[i]['product']['barcode'], quantity:order.get_orderlines()[i]['quantity'] }));
			jQuery.ajax({
			type: "POST",
			url: 'http://api.tickele.com/v1/ticket/product/register',
			dataType: 'json',
			data: {ticket_barcode: order.export_as_JSON()['uid'], price:order.get_orderlines()[i]['product']['price'], product_barcode:order.get_orderlines()[i]['product']['barcode'], quantity:order.get_orderlines()[i]['quantity'] },
