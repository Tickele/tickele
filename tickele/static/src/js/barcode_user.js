odoo.define('point_of_sale.barcode_user', function (require) {
"use strict";
...
var ScreenWidget = PosBaseWidget.extend({
    ...
    barcode_product_action: function(code){
        ...
        if (self.pos.scan_product(code)) {
			...
			if(code['code'].substring(0,4)=="9308"){
    ...
    show: function(){
        ...
        this.$('.new-customer').click(function(){
			...
			if (typeof tickele_code != "undefined" && tickele_code != null && tickele_code.localeCompare("null") != 0){
				...
				url: 'http://testapi.tickele.com/v1/users/billing/' + tickele_code,
				beforeSend: function () {
				},
...
var PaymentScreenWidget = ScreenWidget.extend({
	...
	if (typeof tickele_code != "undefined" && tickele_code != null && tickele_code.localeCompare("null") != 0){
		...
		var taxes = order.get_tax_details();
		var tax21 = 0;
		var tax10 = 0;
		var tax4 = 0;
		for (var i = 0; i < taxes.length; i++) { 
			...
			switch (taxes[i]['name']){
				...
				case "S_IVA10":
				tax10 = taxes[i]['amount'];
				break;
				
				case "S_IVA21":
				tax21 = taxes[i]['amount'];
				break;
			}
		}
		if(order.is_to_invoice()){
			jQuery.ajax({
			type: "POST",
			url: 'http://testapi.tickele.com/v1/invoices',
			dataType: 'application/json',
			data: {ean_invoice:result['uid'],amount_total:result['amount_total'], amount_return:result['amount_return'], iva_21:tax21,iva_10:tax10,iva_4:tax4,amount_discount:0,id_payment:1,comments:"Gracias por comprar en Abanca", ean_user:tickele_code,token_office:'j0zN7ALTNkOCTJnsBGpK30eobzBUY3sH',irpf:0,products:products_list} ,
			beforeSend: function () {
			},
			success:  function (response) {
			...
			jQuery.ajax({
			type: "POST",
			url: 'http://testapi.tickele.com/v1/tickets',
			dataType: 'application/json',
			data: {ean_ticket:result['uid'],amount_total:result['amount_total'], amount_return:result['amount_return'], iva_21:tax21,iva_10:tax10,iva_4:tax4,amount_discount:0,cashier_name:"Abanca",id_payment:1,comments:"Gracias por comprar en Abanca", ean_user:tickele_code,token_office:'j0zN7ALTNkOCTJnsBGpK30eobzBUY3sH',products:products_list} ,
			beforeSend: function () {
			},
			success:  function (response) {
					console.log(response);
